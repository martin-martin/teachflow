...
def run_ocr_mock_for_id(student_id: str, pdf_path: Path | None = None) -> str:
    ...
    - Uses cfg.DEMO_OCR_PAIRS to return a specific OCR text for a given student_id.
    ...
    """
    sid = str(student_id)
    for pair_id, text in cfg.DEMO_OCR_PAIRS:
        if str(pair_id) == sid:
...
def get_demo_ocr_pairs() -> List[Tuple[str, str]]:
    """
    Return the list of (student_id, ocr_text) pairs defined in cfg.DEMO_OCR_PAIRS.

    This is used for CLI / pipeline testing: run_llm_pipeline(get_demo_ocr_pairs(), students_map)
    """
    return [(str(sid), str(text)) for sid, text in cfg.DEMO_OCR_PAIRS]
